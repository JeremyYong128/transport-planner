...
class UserRequirementsSchema(BaseIOSchema):
    ...
    departure_time: str = Field(
        ...,
        description="The time of the user's travel, either in HH:MM format or a textual description.",
    )
...
class RequirementsGeneratorOutputSchema(BaseIOSchema):
    ...
    user_requirements: UserRequirementsSchema = Field(
        ...,
        description="A dictionary containing the user's travel requirements. It should have at least 1 key-value pair.",
    )
...
requirements_generator_agent = BaseAgent(
    BaseAgentConfig(
        client=instructor.from_openai(
            openai.OpenAI()  # TODO: Is initialising necessary?
        ),
        model="gpt-4o-mini",
...
requirements_generator_agent.register_context_provider(
...
if __name__ == "__main__":
    ...
    console.print(
        Panel(
            requirements_generator_agent.system_prompt_generator.generate_prompt(),
            title="System Prompt",
            expand=False,
        )
    )
    ...
    output_schema = Syntax(
        str(requirements_generator_output.model_dump_json(indent=2)),
        "json",
        theme="monokai",
        line_numbers=True,
    )
